<?php

/**
 * @file Внимание! Файл сгенерирован автоматически. Не вносить правки.
 */

declare(strict_types=1);

namespace Cargomart\ApiClient\Entity\Order\Common\Responses;

use Cargomart\ApiClient\AbstractEntity;
use Cargomart\ApiClient\Entity\Order\Common\Objects\OrderTruck;

/**
 * source: order/Common/responses/order-truck-response.json#/properties/data
 *
 * @property OrderTruck[] $trucks
 */
final class OrderTruckResponseData extends AbstractEntity
{
    protected static $types = ['trucks' => ['array', 'Cargomart\ApiClient\Entity\Order\Common\Objects\OrderTruck']];
    protected static $nullables = ['trucks' => false];
}
